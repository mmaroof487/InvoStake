...
export default function Navbar() {
	...
	const NavLink = (prop) =>
		prop.text === "AI Agent" ? (
			<button
				onMouseOver={() => setActiveTab(prop.text)}
				onClick={() => {
					handleScrollToBottom();
				}}
				className={`py-2 px-6 transition-all duration-300 relative ${activeTab === prop.text ? "text-blue-500" : "text-gray-400 hover:text-gray-200"}`}>
				{prop.text}
				{activeTab === prop.text && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-blue-500 to-purple-500" />}
			</button>
		) : (
			<button onMouseOver={() => setActiveTab(prop.text)} className={`py-2 px-6 transition-all duration-300 relative ${activeTab === prop.text ? "text-blue-500" : "text-gray-400 hover:text-gray-200"}`}>
				{prop.text}
				{activeTab === prop.text && <span className="absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-blue-500 to-purple-500" />}
			</button>
		);
